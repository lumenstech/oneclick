...
func TestIdentityPersists(t *testing.T) {
	...
	if enrolled { t.Fatal("unexpected enrolled state") }
	...
	if err != nil { t.Fatal(err) }
	if !enrolled || id2.State.NodeID != "node_1" { t.Fatal("state not loaded") }
	if string(id.Private) != string(id2.Private) { t.Fatal("private key changed") }
}
