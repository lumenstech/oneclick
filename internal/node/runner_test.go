...
func TestRunnerNeverNeedsShellAndBindsApprovedPlan(t *testing.T) {
	...
	if err := r.Deploy(context.Background(), source, "acme/demo", sha, hash, 3000); err != nil {
		t.Fatal(err)
	}
	b, _ := os.ReadFile(logPath)
	got := strings.Split(strings.TrimSpace(string(b)), "\n")
	want := []string{
		"deploy", source, "--yes", "--port=3000",
...
func TestRunnerDoesNotForwardParentSecretsOrChildOutput(t *testing.T) {
	...
	if strings.Contains(err.Error(), "repo-secret-value") || strings.Contains(err.Error(), "must-not-reach-child") {
		t.Fatalf("error leaked child or parent secret: %v", err)
	}
	...
	if len(b) != 0 {
		t.Fatalf("parent secret reached child environment: %q", string(b))
	}
}
