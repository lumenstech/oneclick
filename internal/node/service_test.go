...
func (f *fakeControl) SendResult(_ context.Context, r CommandResult) error { f.results = append(f.results, r); return nil }
...
func TestExecuteAtMostOnceAndResendsCachedResult(t *testing.T) {
	...
	if err := svc.execute(context.Background(), cmd); err != nil { t.Fatal(err) }
	if err := svc.execute(context.Background(), cmd); err != nil { t.Fatal(err) }
	if fetch.calls != 1 || run.deploy != 1 || ctl.credentialCalls != 1 { t.Fatalf("command executed more than once: fetch=%d deploy=%d creds=%d", fetch.calls, run.deploy, ctl.credentialCalls) }
	if run.repo != cmd.Deploy.Repository || run.revision != cmd.Deploy.CommitSHA || run.planHash != cmd.PlanHash { t.Fatalf("approved plan identity not bound to runner: repo=%q rev=%q hash=%q", run.repo, run.revision, run.planHash) }
	if len(ctl.results) != 2 || ctl.results[0].Status != "succeeded" || ctl.results[1].Status != "succeeded" { t.Fatalf("cached result not resent: %#v", ctl.results) }
}
